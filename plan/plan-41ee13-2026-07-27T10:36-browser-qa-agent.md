...
- 2026-07-27 — RED stage: committed RED prerequisites (`projects:` runner table in devx.config.yaml — api/app/e2e/workstream-evals; `_devx/workstreams/run-eval.sh` dispatcher). Authored all 6 eval artifacts at exact Verified-by paths: `e1_runner_resolution.sh` (dry-run JSON parse + playwright grep), `e2_e2e_one_command.sh` (missing-feature checks before infra checks — cheap RED without Docker), `e3_walkthrough_emission.sh` (pins the machine/human item parse contract for the Phase 3 template), `e5_red_browser_flow.sh` (wrapper asserting README + demo shape), E-4/E-6 human stubs. Dry-run sanity: 4 planned (all with commands via workstream-evals runner) + 2 deferred (human). `devx gate evals 41ee13` → PASS — all 4 right-reason (`MISSING BEHAVIOR:` banners, zero `INFRA:`), report `evals/RED-report.md`; flipped evals_red ✓, stage → executing. Emitted dev specs bqa101–bqa107 (one per phase, deps 1∥2∥3 → 4 → 5 → 6 → 7) + retro `bqaret` via emit-retro-story; DEV.md epic section appended (note: `###` header per emit-retro-story's section contract; pre-existing hand-imported sections use `##`); validate-emit ok.
- 2026-09-22 — workstream artifacts moved to the folder-per-artifact layout (`_devx/workstreams/browser-qa-agent/{prd,design,plan}.md` → `…/{prd,design,plan}/agent.md`) by `devx doctor`'s content-safe repair. Paths in earlier lines of this log are historical.
