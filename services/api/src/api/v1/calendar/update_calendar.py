...
class UpdateCalendar(AsyncEndpoint):
    """Update a calendar's name or description (owner-only)."""

    async def execute(self, calendar_id: str, params: "UpdateCalendar.Params"):
        user: User = self.user
        ...
        if updates:
            await self.database.update(calendar, **updates)

        member_count = (
            await self.db.execute(
                select(func.count(CalendarUser.user_id)).where(
