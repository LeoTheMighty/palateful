...
def _count_by_status(db, job_id, status: str) -> int:
    return (
        db.query(ImportItem)
        .filter(
...
def recompute_import_job_counters(db, job: ImportJob) -> None:
    ...
    """
    job.succeeded_items = _count_by_status(db, job.id, "completed")
    job.failed_items = _count_by_status(db, job.id, "failed")
    job.pending_review_items = _count_by_status(db, job.id, "awaiting_review")
