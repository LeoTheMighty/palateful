...
aam-11: handlers are `AsyncEndpoint`; tests use `mock_async_db`.
"""
...
def _configure_add_member_mocks(mock_async_db, owner_membership, target_user, book):
    """Wire up the set_find_by calls shared by every happy-path add_member test.

    AddRecipeBookMember does three `find_by` lookups (caller owner, target
    user, existing membership) plus one inside the router for the
    notification fan-out (`RecipeBook` by id). `set_find_by` keys the
    stored result by model + sorted kwargs, so a given lookup matches
    both the endpoint and router callsites.
    """
    mock_async_db.set_find_by(RecipeBookUser, owner_membership,
                              user_id=str(owner_membership.user_id),
                              recipe_book_id=BOOK_ID)
    mock_async_db.set_find_by(User, target_user, id=TARGET_USER_ID)
...
class TestAddRecipeBookMember:
    ...
    def test_owner_can_add_editor(self, client, mock_async_db, mock_user):
        ...
        _configure_add_member_mocks(
            mock_async_db, owner_membership, target_user, book
        )
    ...
    def test_owner_can_add_viewer(self, client, mock_async_db, mock_user):
    ...
    def test_editor_cannot_add_member(self, client, mock_async_db, mock_user):
        ...
        mock_async_db.set_find_by(RecipeBookUser, editor_membership,
    ...
    def test_viewer_cannot_add_member(self, client, mock_async_db, mock_user):
        ...
        mock_async_db.set_find_by(RecipeBookUser, viewer_membership,
                                  user_id=str(mock_user.id), recipe_book_id=BOOK_ID)
    ...
    def test_invalid_role_rejected(self, client, mock_async_db, mock_user):
    ...
    def test_no_access_returns_403(self, client, mock_async_db, mock_user):
...
class TestRemoveRecipeBookMember:
    ...
    def test_owner_can_remove_member(self, client, mock_async_db, mock_user):
        ...
        mock_async_db.set_find_by(RecipeBookUser, owner_membership,
                                  user_id=str(mock_user.id), recipe_book_id=BOOK_ID)
        mock_async_db.set_find_by(RecipeBookUser, target_membership,
    ...
    def test_editor_cannot_remove_member(self, client, mock_async_db, mock_user):
    ...
    def test_owner_cannot_remove_self(self, client, mock_async_db, mock_user):
...
class TestUpdateRecipeBookMemberRole:
    ...
    def test_owner_can_update_role(self, client, mock_async_db, mock_user):
    ...
    def test_editor_cannot_update_role(self, client, mock_async_db, mock_user):
...
class TestListRecipeBooksRoleInfo:
    ...
    def test_list_includes_user_role_and_is_shared(self, client, mock_async_db, mock_user):
    ...
    def test_list_with_shared_book(self, client, mock_async_db, mock_user):
        ...
        mock_async_db.db.execute.side_effect = [
            ...
            MockExecuteResult(items=[(book, 2, "editor", 3, datetime.now(UTC))]),
        ]
...
class TestGetRecipeBookRoleInfo:
    ...
    def test_get_includes_user_role_and_members(self, client, mock_async_db, mock_user):
    ...
    def test_get_no_access_returns_403(self, client, mock_async_db, mock_user):
...
class TestAddRecipeBookMemberMissingBranches:
    ...
    def test_add_member_user_not_found(self, client, mock_async_db, mock_user):
    ...
    def test_add_member_already_exists(self, client, mock_async_db, mock_user):
        ...
        mock_async_db.set_find_by(RecipeBookUser, owner_membership,
                                  user_id=str(mock_user.id), recipe_book_id=BOOK_ID)
        mock_async_db.set_find_by(User, target_user, id=TARGET_USER_ID)
        mock_async_db.set_find_by(RecipeBookUser, existing_membership,
                                  user_id=TARGET_USER_ID, recipe_book_id=BOOK_ID)
...
class TestRemoveRecipeBookMemberMissingBranches:
    ...
    def test_remove_member_not_found(self, client, mock_async_db, mock_user):
    ...
    def test_remove_member_no_access(self, client, mock_async_db, mock_user):
...
class TestUpdateRecipeBookMemberRoleMissingBranches:
    ...
    def test_owner_cannot_change_own_role(self, client, mock_async_db, mock_user):
    ...
    def test_update_role_member_not_found(self, client, mock_async_db, mock_user):
    ...
    def test_update_role_no_access(self, client, mock_async_db, mock_user):
