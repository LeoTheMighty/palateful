...
Query shape is asserted via the `mock_async_db.db.execute` call args — we
...
class TestGetClientRouteMetrics:
    def test_returns_rows(self, client, mock_user, mock_async_db):
    ...
    def test_null_p95_tolerated(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_async_db.db.execute.return_value = MockExecuteResult([
    ...
    def test_empty(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_async_db.db.execute.return_value = MockExecuteResult([])
    ...
    def test_invalid_window(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        resp = client.get("/v1/admin/metrics/client/routes?window=365d")
        assert resp.status_code == 400

    def test_accepts_all_four_windows(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
    ...
    def test_filters_propagate_to_sql_params(
        ...
        assert resp.status_code == 200
        params = _last_execute_args_dict(mock_async_db)
        assert params.get("platform") == "ios", params
        assert params.get("app_version") == "1.2.3"
        assert params.get("route") == "/home"

    def test_non_admin_gets_403(self, client, mock_user, mock_async_db):
...
class TestGetClientJankMetrics:
    ...
    def test_null_percentiles(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_async_db.db.execute.return_value = MockExecuteResult([
...
class TestGetClientSparkline:
    def test_returns_24_buckets(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_async_db.db.execute.return_value = MockExecuteResult([
    ...
    def test_invalid_metric(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        resp = client.get(
    ...
    def test_all_filter_combinations(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_async_db.db.execute.return_value = MockExecuteResult([])
        resp = client.get(
            "/v1/admin/metrics/client/sparkline"
...
def _last_execute_call(mock_async_db):
    return mock_async_db.db.execute.call_args


def _last_execute_args_dict(mock_async_db):
    args, _ = mock_async_db.db.execute.call_args
