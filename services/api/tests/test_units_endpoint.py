...
def _stub_two_queries(mock_db, *, alias_rows, canonical_rows):
    """Configure mock_db.db.execute to return alias rows then canonical rows.

    The endpoint issues two execute() calls in order:
...
def test_get_unit_aliases_returns_map_and_canonical(client, mock_db):
    _stub_two_queries(
        mock_db,
...
def test_get_unit_aliases_emits_24h_cache_control(client, mock_db):
    _stub_two_queries(mock_db, alias_rows=[], canonical_rows=[])
    response = client.get("/v1/units/aliases")
    assert response.status_code == 200
...
def test_get_unit_aliases_handles_empty_seed(client, mock_db):
    _stub_two_queries(mock_db, alias_rows=[], canonical_rows=[])
    response = client.get("/v1/units/aliases")
    assert response.status_code == 200
