...
def _make_endpoint(mock_db, mock_user):
    from api.v1.search.unified_search import UnifiedSearch

    return UnifiedSearch(user=mock_user, database=mock_db)
...
class TestSearchMyMeals:
    ...
    def test_zero_book_ids_short_circuits(self, mock_db, mock_user):
        endpoint = _make_endpoint(mock_db, mock_user)
        # book_ids=[] — no DB calls at all
        results = endpoint._search_my_meals("dressing", 20, mock_user, [])
        assert results == []
        assert mock_db.db.execute.call_count == 0

    def test_direct_name_match_populates_null_matched_component(
        ...
        results = endpoint._search_my_meals(
    ...
    def test_component_name_match_populates_matched_component(
        self, mock_db, mock_user
    ):
        endpoint = _make_endpoint(mock_db, mock_user)
        meal = _MockMeal(
    ...
    def test_direct_and_component_both_return_direct_first(
        self, mock_db, mock_user
    ):
        endpoint = _make_endpoint(mock_db, mock_user)
    ...
    def test_direct_fills_limit_skips_component_query(
        self, mock_db, mock_user
    ):
        ...
        mock_db.db.execute.return_value = MockExecuteResult([(meal, "Dinners")])
        results = endpoint._search_my_meals(
    ...
    def test_archived_recipe_component_excluded_from_image_urls(
        self, mock_db, mock_user
    ):
        endpoint = _make_endpoint(mock_db, mock_user)
        meal = _MockMeal(
        ...
        results = endpoint._search_my_meals("Kale", 20, mock_user, ["book-1"])
        assert len(results[0].top_component_image_urls) == 1
        assert "r1" in results[0].top_component_image_urls[0]

    def test_component_match_no_exact_component_name_hit_yields_null_matched(
        self, mock_db, mock_user
    ):
    ...
    def test_top_image_urls_capped_at_four(self, mock_db, mock_user):
        endpoint = _make_endpoint(mock_db, mock_user)
        meal = _MockMeal(
            components=[
        ...
        results = endpoint._search_my_meals("x", 20, mock_user, ["book-1"])
        assert len(results[0].top_component_image_urls) == 4

    def test_archived_book_component_excluded(self, mock_db, mock_user):
        ...
        results = endpoint._search_my_meals("K", 20, mock_user, ["book-1"])
        # archived-book component is filtered; only "Fresh" leaks its URL
        assert len(results[0].top_component_image_urls) == 1
        assert "r2" in results[0].top_component_image_urls[0]

    def test_component_without_image_url_skipped(self, mock_db, mock_user):
...
class TestUnifiedSearchScopeIntegration:
    ...
    def test_scope_recipes_excludes_my_meals(self, client, mock_db, mock_user):
        mock_db.db.query.return_value = MockQuery([])
        mock_db.db.execute.return_value = MockExecuteResult([])
