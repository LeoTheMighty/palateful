...
class TestSendTestPushHappyPath:
    def test_success_writes_audit_row_and_returns_ok(self, client, mock_user, mock_async_db):
...
class TestSendTestPushLogOnlyMode:
    def test_log_only_returns_ok_with_flag_set(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_async_db.db.execute.return_value = MockExecuteResult([mock_user])
...
class TestSendTestPushQuietHours:
    def test_force_false_during_quiet_hours_reports_suppression(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_user.push_tokens = ["token-abc"]
        mock_async_db.db.execute.return_value = MockExecuteResult([mock_user])
...
class TestSendTestPushNoTokens:
    def test_no_tokens_registered_returns_specific_outcome(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
...
class TestSendTestPushRateLimit:
    def test_11th_request_returns_429_no_audit_row(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        mock_user.push_tokens = ["token-abc"]
        mock_async_db.db.execute.return_value = MockExecuteResult([mock_user])
        ...
        with patch(
            ...
            pre_rate_limit_adds = len(mock_async_db.db.add.call_args_list)
            ...
            assert len(mock_async_db.db.add.call_args_list) == pre_rate_limit_adds
            # And the push service was NOT called
            assert len(fake.calls) == 10


class TestSendTestPushTargetUserNotFound:
    def test_unknown_target_user_id_returns_404(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
...
class TestSendTestPushExplicitTarget:
    def test_with_custom_title_body_targeting_another_user(self, client, mock_user, mock_async_db):
        mock_user.is_admin = True
        target_user = MockUser(id="11111111-1111-1111-1111-111111111111", push_tokens=["tok"])
        mock_async_db.db.execute.return_value = MockExecuteResult([target_user])
