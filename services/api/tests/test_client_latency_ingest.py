...
def authed_client(mock_db, mock_user):
    """TestClient where get_optional_user returns the mock user."""
...
class TestEventCap:
    def test_accepts_at_cap(self, authed_client, mock_db):
        ...
        mock_db.db.bulk_insert_mappings.assert_called_once()

    def test_over_cap_returns_413(self, authed_client, mock_db):
        ...
        mock_db.db.bulk_insert_mappings.assert_not_called()

    def test_empty_batch_is_a_noop(self, authed_client, mock_db):
        response = authed_client.post(
...
class TestAuthedPath:
    def test_user_id_derived_from_current_user(
        self, authed_client, mock_db, mock_user
    ):
        response = authed_client.post(
            "/v1/client-latencies",
            json={"events": [_event()]},
        ...
        call = mock_db.db.bulk_insert_mappings.call_args
        rows = call.args[1]
...
class TestAnonymousPath:
    def test_anon_insert_persists_null_user_id(self, anon_client, mock_db):
        response = anon_client.post(
            "/v1/client-latencies",
            json={"events": [_event()]},
        )
        assert response.status_code == 200
        rows = mock_db.db.bulk_insert_mappings.call_args.args[1]
...
class TestRouteRedactionGuard:
    ...
    def test_unredacted_route_returns_422(
        self, authed_client, mock_db, route
    ):
        response = authed_client.post(
            "/v1/client-latencies",
    ...
    def test_422_on_first_bad_event_short_circuits(
        ...
        sync bulk insert can't do partial writes."""
        events = [
            _event(route="/home"),
...
class TestSchemaValidation:
    ...
    def test_extra_bag_is_stored_verbatim(self, authed_client, mock_db):
        response = authed_client.post(
            "/v1/client-latencies",
            json={
                "events": [
