...
- 2026-09-23T15:30 — unrelated to this spec's subject but recorded here because it is tonight's other zero-that-reads-as-fine: **a PR can silently stop receiving Actions events.** PR #68 took two pushes, a close/reopen, and an empty commit without producing a single workflow run, while a PR opened from a fresh branch minutes later on the same account picked up 7 check runs immediately. Not a path filter (neither workflow has one), not a global outage (scheduled runs kept firing), not the concurrency group (a cancelled run still appears as a run). Mechanism unknown; the isolation is the finding. What makes it dangerous is the phrasing: `gh pr checks` answers *"no checks reported on the branch"*, which reads like a note about configuration rather than "nothing measured this code". Recovery that worked: open a new PR from the same branch — the branch and its commits were never the problem.
