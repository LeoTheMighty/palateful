...
def utils_database_with_sqlite_url(monkeypatch, tmp_path):
    ...
    # rsh105: the "asyncpg is not pinned" premise above holds for the
    # `libraries/utils` project, but the workspace-root venv the nx target
    # runs in has asyncpg — and then `create_async_engine` rejects the
    # SQLite URL. Blocking the import makes the premise true in every venv
    # (`database.py` probe-imports asyncpg and skips the async engine).
