...
## Addendum — 2026-04-25 — Beat Recime: Parity + Differentiation + Free-Forever Positioning + Recime Mass-Import (5 epics)

**Context.** April 2026 refresh of `investigations/07-competitor-analysis-recime.md` surfaced four planning gaps: (a) Recime quietly cut US pricing to **$39.99/yr** in some cohorts but spent Q1 on a rebrand instead of closing the pantry/dedup/sort gaps that drive 2026 review complaints — strategic opening; (b) NEW competitor **Recipe Notes** ([recipenotes.app](https://recipenotes.app/free-recime-alternative)) explicitly markets as "Free ReciMe Alternative" — Palateful's defensibility shifts from "we're free" to "we're free AND pantry-aware AND household-native AND Meals-capable"; (c) Apple+Gemini Siri arriving late 2026 demoed system-level pantry intelligence — household + shared + pantry moats matter MORE now; (d) Palateful Must-Adopt parity is 5/5, Should-Adopt is 3.6/5, the remaining open gaps are **social-media video import** (Recime's #1 growth driver — `epic-media-import` was deleted, partial work landed) + **pantry write-side hooks + "What can I cook?" ranking** (Palateful's biggest differentiation moat, sitting in backlog) + **nutrition auto-calc** (Recime Premium-gates this, free for us) + **free-forever positioning copy** (zero user-visible marketing surface mentions pricing today). **Mid-planning addition:** user asked whether one-click migration from Recime is feasible; feasibility check returned YELLOW with a pivot — Recime recipes are private (not scrapeable from a server), but a third-party Chrome extension already proves the user-side path works against Recime's own internal API using the user's session cookie. **Pivoted scope: a Palateful-branded Chrome extension that POSTs into our import endpoint** — sidesteps Recime TOS section 2.2(c)/(d) by mirroring Recime's GDPR data-portability promise.

**User-locked decisions (2026-04-25 batch):**
- **Output:** refreshed investigation doc + actionable epics + pricing-strategy artifact (positioning epic).
- **Pricing model:** **free forever, locked.** Strip the "v1 — no in-app purchases" qualifier from `ANDROID.md`. Any future monetization would have to be donations / one-time / non-paywall — closes off any subscription pivot. Strongest hook against Recime's billing-complaint-laden $39.99–$59.99/yr.
- **Parity scope:** match the loved features, leapfrog the disliked ones (~4 epics → 5 with mid-planning Recime-import addition).
...
- **Smart shopping list dedup:** **keep as cut** per the 2026-04-20 decision in `epic-ingredients-string-simplification`. Position Meals as the dedup answer ("bundle related recipes into a Meal, get a deduped list"). Don't reverse a 5-day-old explicit choice without a stronger trigger.
- **Recipe Notes response:** differentiate harder on pantry + household + Meals. Don't fight on free; win on capability stack at $0.
- **Cross-app migration via uploaded export files:** **deferred** this round — acquisition utility, not parity feature. (Distinct from Recime mass-import via Chrome extension — that one IS in scope.)
- **Recime mass-import (mid-planning addition):** Chrome extension MVP path. Lawyer check before public Chrome Web Store launch but not an epic blocker.

Five epics. **Recommended ship order:** positioning first (low effort, locks copy), then social-video + pantry-cook-with-what-you-have in parallel (independent backends), then Recime-mass-import (depends on the duplicate-detection path landing first), then nutrition last.

- [epic-recime-positioning](epic-recime-positioning.md) — **user sees:** opens the App Store / Play Store listing and reads "Free forever. No paywalls, no ads, no premium tier — ever. Unlimited imports. Real household sharing." Lands on a real `palateful.app` web page (replacing today's empty Flutter shell) with a hero, a comparison table (Palateful vs Recime vs Recipe Notes vs Mela), and direct App Store / Play Store download buttons. Inside the app, the onboarding welcome screen carries a single subtle reassurance line — "100% free, no ads, no premium tier — ever." A new "Why we're free" tap target in Settings links to a static page explaining the founder-funded, no-investor-pressure commitment. The privacy policy now affirms "We do not sell or monetize your data, and we're committed to keeping Palateful free indefinitely." Share-recipe cards include "Get Palateful — free forever" tagline. — **Touches:** **frontend** (onboarding reassurance line, new `WhyWeAreFreePage` widget + Settings link, share-card tagline update), **backend** (None — pure content/copy work, no API shape change), **infrastructure** (web landing page deployed via Cloudflare Pages replacing the empty Flutter shell at the root, `app/web/privacy.html` updated with the free-forever affirmation, `ANDROID.md` Section 6 stripped of the "v1" qualifier, App Store Connect / Play Console listing copy + screenshots updated via the existing manual operator runbook).
- [epic-social-video-import](epic-social-video-import.md) — **user sees:** watches a recipe on TikTok / Instagram Reel / YouTube Short, taps native Share → Palateful icon. Sees a non-blocking "Extracting recipe from your video — we'll notify you when ready" sheet that auto-dismisses; the video uploads in the background. ~30-60 seconds later a push notification arrives: "Your recipe from TikTok is ready! Tap to review." Tapping opens the Approve-Import screen with the extracted recipe + a "From @chefname on TikTok" attribution badge + a thumbnail; the user corrects any field, taps Approve, and the recipe lands in **Trying Out** (per `epic-recipe-default-books`). When extraction fails, a friendly "We couldn't extract a recipe from this video — try sharing a screenshot of the recipe text instead" empty state appears with a one-tap fallback to manual entry. — **Touches:** **frontend** (`share_receiving_handler.dart` extension to detect social-media URLs/video files and route to a new `VideoImportLandingScreen`, per-platform thumbnail + multi-stage progress text, Approve-Import attribution badge widget, video-extraction failure empty state), **backend** (new `services/api/src/services/audio_transcription.py` wrapping OpenAI Whisper with cost cap, new platform scrapers `tiktok_scraper.py` + `instagram_scraper.py` + `youtube_scraper.py` with graceful degradation when caption APIs return nothing, new `social_video_extractor.py` orchestrator, `import_items.source_type` enum extension to include `social_video`, `import_items.metadata` JSON column expansion for video attribution {platform, creator handle, caption text, video duration}, new `extract_recipe_from_video_task` worker task queued from share-sheet upload completion, push notification on completion via existing notification system), **infrastructure** (worker container ffmpeg already present per `sbf-4`; reuses existing `OPENAI_API_KEY`; optional new env vars `INSTAGRAM_API_TOKEN` + `YOUTUBE_API_KEY` for caption fetching with degraded path if absent; per-import cost logged via existing `error_logs` audit pattern; no new AWS resources). **No dependencies** beyond shipped share-sheet infrastructure.
- [epic-pantry-cook-with-what-you-have](epic-pantry-cook-with-what-you-have.md) — **user sees:** completes a shopping run (checks off items at the store) and the matching items now auto-add to their pantry with a "purchased today" date — settings toggle controls whether this is automatic or prompted. Cooks a recipe via cook mode + completes the post-cook flow, and the screen asks "Update pantry — used 200g chicken, 1 onion, 3 cloves garlic? [Yes] [Skip]" defaulted to Yes; tapping Yes decrements those pantry items (or marks them used up if depleted). Opens the home screen and sees a new "What Can I Cook Tonight?" card showing recipes ranked by pantry coverage — each with "You have all 8 ingredients" or "You're missing 2: heavy cream, basil — [Add to shopping list]." Recipe detail screens carry a small pantry-coverage badge ("You have 6 of 8 ingredients"). Use-it-up nudges fire when pantry items approach expiry: "Your chicken thighs expire Friday — here are 3 quick recipes that use them." — **Touches:** **frontend** (new "What Can I Cook?" home card + cookable-recipes provider, pantry-coverage badge on recipe detail, post-cook pantry-update confirmation sheet defaulting to Yes, shopping-list checkout long-press "Add to pantry" optional action, settings toggle for auto-add behavior, use-it-up nudge on home screen when pantry items near expiry), **backend** (new `services/api/src/services/pantry_decrement.py` computing per-item deltas given a recipe + serving count + unit normalization, new `POST /v1/pantry/apply-cook-completion` endpoint called from cook mode finish, new `POST /v1/pantry/apply-shopping-checkout` endpoint called from shopping-list check-off with user-preference flag, new `GET /v1/recipes/cookable` endpoint with ranking algorithm + ingredient-diff response shape, new `GetCookableRecipesTool` for the AI agent + MCP server, schema additions `pantry_items.last_used_at` + `pantry_items.purchased_at` for "use me up" sorting), **infrastructure** (None — existing pantry tables, existing endpoint patterns, no new env vars or AWS resources). **No dependencies** beyond pantry CRUD already shipped.
- [epic-recime-mass-import](epic-recime-mass-import.md) — **user sees:** opens Settings → "Import from another app" → taps "Recime" and lands on a friendly walkthrough that explains the three-step flow with annotated screenshots: (1) install our Chrome extension via a one-tap "Install Extension" button that deep-links to the Chrome Web Store, (2) log in to recime.app in your own browser, (3) click the Palateful extension button in your toolbar and choose "Send all my recipes to Palateful." A short FAQ on the same screen explains: "Why a browser extension? Because Recime doesn't expose a public API — your browser does the work, your data stays yours." Once the extension fires, the Activity Hub immediately shows a new long-running import job row ("Importing from Recime — 12 of 87 recipes") with the same YNAB-inspired status icons used elsewhere; tapping it shows per-recipe success / skip-as-duplicate / fail breakdown in real-time. ~60-90 seconds later a push notification fires: "87 recipes imported from Recime — review in Trying Out." Tapping the notification deep-links into the user's `Trying Out` book where the imported recipes appear, ready to organize, cook, or move into other books. — **Touches:** **frontend** (new `RecimeImportWalkthroughScreen` in Settings with annotated screenshots + "Install Extension" CTA + FAQ accordion; Activity Hub long-running-job row for Recime imports with per-recipe success/skip/fail counts; push-notification deep-link routing to `Trying Out`), **backend** (new `POST /v1/recipes/import/recime` endpoint authenticated via existing Palateful API token, accepting a Recime-shaped JSON payload, normalizing to Palateful's recipe model, filing into `Trying Out` per `epic-recipe-default-books`, deduping via the existing duplicate-detection path per `epic-import-duplicate-detection`; rate-limited to 200 requests / 5 minutes per user; per-session start/end audit rows in `error_logs` capturing intended count + final success/skip/fail; per-user cap of 2 mass-import sessions / 24h with graceful UX on the third), **infrastructure** (new Chrome extension distributed via the Chrome Web Store under Palateful's developer account — Manifest V3, content script injected into recime.app, popup with single primary CTA, paginated fetch of Recime's internal list-recipes endpoint, per-recipe POST to Palateful with the user's API token stored in `chrome.storage.local`; no new AWS resources, no new env vars on the FastAPI side; one new Chrome Web Store listing). **Depends on** `epic-recipe-default-books` (Trying Out destination — already shipping per recent /dev commits) and `epic-import-duplicate-detection` (dedup path — also shipping). Lawyer check on TOS framing required before public Chrome Web Store launch.
- [epic-nutrition-auto-calc](epic-nutrition-auto-calc.md) — **user sees:** opens any recipe (imported or manually created) and finds a new nutrition card below the ingredients showing "Per serving: 420 cal · 32g protein · 38g carbs · 18g fat" with a small "* Estimated based on USDA data" disclaimer. Tapping the card expands to a per-ingredient breakdown sheet showing how each ingredient contributes. When an ingredient can't be matched to USDA data, the card shows "Nutrition unavailable for: heavy cream — [enter manually]" with an inline manual-entry path. On the recipe edit screen, optional fields let the user override the calculated values per serving (calories / protein / carbs / fat) — useful when the USDA estimate disagrees with their judgement. A Settings toggle controls whether nutrition cards display globally (default on). — **Touches:** **frontend** (new `NutritionCard` widget on recipe detail, expanded breakdown sheet, manual-macro override fields on recipe edit, missing-ingredient empty state, Settings toggle "Show nutrition" defaulting to on), **backend** (Alembic migration extending `ingredients` table with `nutrition_per_unit` JSON column {calories, protein_g, carbs_g, fat_g per standard unit + density factor for unit conversion}, one-time data-load script `services/api/scripts/load_usda_nutrition.py` populating ~5000 most-common ingredients from USDA FoodData Central, second migration extending `recipes` with `nutrition_per_serving` JSON computed field + `manual_nutrition_override` JSON, new `services/api/src/services/nutrition_calculator.py` summing ingredient nutrition × quantity with unit normalization divided by servings, recalc-on-save hook on recipe + ingredient edit paths, `GET /v1/recipes/{id}` response shape addition for `nutrition_per_serving`), **infrastructure** (one-time USDA CSV data load via the new ops script run on staging then prod after audit-row sign-off; two Alembic migrations; no new AWS resources, no new env vars). **No dependencies**, but recommended to ship after `epic-social-video-import` so freshly-extracted recipes pick up nutrition immediately.

All five epics share the **free-forever positioning bar:** any user-visible copy or surface produced under any epic must honor the free-forever commitment. No "premium," "pro," "upgrade," or "unlock" language. If a story discovers it needs to gate something, the gate must be removed (or the feature cut) — not a paywall added.


## Addendum — 2026-04-25 — Beat Recime (5 epics)

**Context.** April 2026 refresh of `investigations/07-competitor-analysis-recime.md`. Recime cut US pricing to $39.99/yr in some cohorts but spent Q1 on a rebrand — pantry / merge-duplicates / sort / public-recipes complaints all still open. NEW competitor Recipe Notes ([recipenotes.app](https://recipenotes.app/free-recime-alternative)) markets as "Free ReciMe Alternative" — Palateful's defensibility shifts from "we're free" to "we're free AND pantry-aware AND household-native AND Meals-capable." Apple+Gemini Siri late 2026 makes household + shared + pantry moats matter MORE. Mid-planning addition: Recime mass-import feasibility check returned YELLOW with a Chrome-extension pivot using the user's own session cookie. Full PRD addendum at `prd.md` (FR-COMP-1 through FR-COMP-16).

**User-locked decisions (2026-04-25):** free forever locked (strip "v1" qualifier from ANDROID.md), parity scope = match loved + leapfrog disliked, social-video full pipeline, shopping-list dedup stays cut, Recipe Notes response = differentiate-on-stack not fight-on-price, cross-app uploaded-file migration deferred, Recime Chrome extension YES with lawyer check pre-launch.

**Recommended ship order:** positioning first (locks copy), then social-video + pantry-cook in parallel (independent backends), then Recime-mass-import (depends on duplicate-detection landing first), then nutrition last.

- [epic-recime-positioning](epic-recime-positioning.md) — **user sees:** opens App Store / Play Store listing reading "Free forever. No paywalls, no ads, no premium tier — ever. Unlimited imports. Real household sharing." Lands on real `palateful.app` web page (replacing today's empty Flutter shell) with hero, comparison table (Palateful vs Recime vs Recipe Notes vs Mela), App Store / Play Store download buttons. In-app onboarding welcome screen carries subtle reassurance line — "100% free, no ads, no premium tier — ever." New "Why we're free" Settings tap target → static page. Privacy policy affirms "We do not sell or monetize your data, and we're committed to keeping Palateful free indefinitely." Share-recipe cards include "Get Palateful — free forever" tagline. — **Touches:** **frontend** (onboarding reassurance line, new `WhyWeAreFreePage` widget + Settings link, share-card tagline), **backend** (None — content/copy only), **infrastructure** (web landing at `palateful.app` via Cloudflare Pages replacing Flutter shell, `app/web/privacy.html` updated, `ANDROID.md` Section 6 stripped of "v1" qualifier, App Store / Play Console listing copy + screenshots updated via existing operator runbook). **No dependencies.**
- [epic-social-video-import](epic-social-video-import.md) — **user sees:** watches a recipe on TikTok / Instagram Reel / YouTube Short, taps Share → Palateful icon. Non-blocking "Extracting recipe from your video — we'll notify you when ready" sheet auto-dismisses; video uploads in background. ~30-60 sec later push notification: "Your recipe from TikTok is ready! Tap to review." Tapping opens Approve-Import with extracted recipe + "From @chefname on TikTok" attribution + thumbnail. User corrects, taps Approve, recipe lands in `Trying Out`. Extraction failure → friendly empty state with screenshot-import fallback. — **Touches:** **frontend** (`share_receiving_handler.dart` extension to detect social-media URLs/video files + route to new `VideoImportLandingScreen`, per-platform thumbnail + multi-stage progress text, Approve-Import attribution badge, video-extraction failure empty state), **backend** (new `services/api/src/services/audio_transcription.py` wrapping OpenAI Whisper with cost cap, new platform scrapers `tiktok_scraper.py` + `instagram_scraper.py` + `youtube_scraper.py` with graceful degradation, new `social_video_extractor.py` orchestrator, `import_items.source_type` enum extension to include `social_video`, `import_items.metadata` JSON column expansion for video attribution, new `extract_recipe_from_video_task` worker task, push notification on completion), **infrastructure** (worker container ffmpeg already present per `sbf-4`; reuses existing `OPENAI_API_KEY`; optional `INSTAGRAM_API_TOKEN` + `YOUTUBE_API_KEY` env vars with degraded path if absent; per-import cost logged via `error_logs` audit; no new AWS resources). **No dependencies** beyond shipped share-sheet infra.
- [epic-pantry-cook-with-what-you-have](epic-pantry-cook-with-what-you-have.md) — **user sees:** completes shopping run (checks off items at store) → matching items auto-add to pantry with "purchased today" date (Settings toggle controls auto vs prompted). Cooks recipe via cook mode + completes post-cook flow → screen asks "Update pantry — used 200g chicken, 1 onion, 3 cloves garlic? [Yes] [Skip]" defaulted to Yes; tapping Yes decrements pantry items (or marks used up if depleted). Home screen shows new "What Can I Cook Tonight?" card ranking recipes by pantry coverage — each "You have all 8 ingredients" or "You're missing 2: heavy cream, basil — [Add to shopping list]." Recipe detail carries small pantry-coverage badge ("You have 6 of 8 ingredients"). Use-it-up nudges fire when pantry items approach expiry: "Your chicken thighs expire Friday — here are 3 quick recipes that use them." — **Touches:** **frontend** (new "What Can I Cook?" home card + cookable-recipes provider, pantry-coverage badge on recipe detail, post-cook pantry-update confirmation sheet defaulting to Yes, shopping-list checkout long-press "Add to pantry" optional action, Settings toggle for auto-add behavior, use-it-up nudge), **backend** (new `services/api/src/services/pantry_decrement.py` computing per-item deltas given a recipe + serving count + unit normalization, new `POST /v1/pantry/apply-cook-completion` + `POST /v1/pantry/apply-shopping-checkout` endpoints, new `GET /v1/recipes/cookable` with ranking + ingredient-diff response, new `GetCookableRecipesTool` for AI agent + MCP, schema additions `pantry_items.last_used_at` + `pantry_items.purchased_at`), **infrastructure** (None — existing pantry tables, existing endpoint patterns, no new env vars or AWS). **No dependencies** beyond pantry CRUD already shipped.
- [epic-recime-mass-import](epic-recime-mass-import.md) — **user sees:** opens Settings → "Import from another app" → "Recime" → walkthrough with annotated screenshots: (1) install our Chrome extension via one-tap "Install Extension" CTA deep-linking to Chrome Web Store, (2) log in to recime.app in your own browser, (3) click Palateful extension button + "Send all my recipes to Palateful." FAQ: "Why a browser extension? Because Recime doesn't expose a public API — your browser does the work, your data stays yours." Activity Hub immediately shows long-running import job row ("Importing from Recime — 12 of 87 recipes") with YNAB-inspired status icons; tapping shows per-recipe success/skip-as-duplicate/fail breakdown in real-time. ~60-90 sec later push notification: "87 recipes imported from Recime — review in Trying Out." Tap deep-links into user's `Trying Out` book where imported recipes appear, ready to organize/cook/move. — **Touches:** **frontend** (new `RecimeImportWalkthroughScreen` in Settings with annotated screenshots + "Install Extension" CTA + FAQ; Activity Hub long-running-job row for Recime imports with per-recipe counts; push-notification deep-link routing to `Trying Out`), **backend** (new `POST /v1/recipes/import/recime` endpoint authed via existing Palateful API token, accepting a Recime-shaped JSON payload, normalizing to Palateful's recipe model, filing into `Trying Out`, deduping via existing duplicate-detection path; rate-limited 200 req / 5 min per user; per-session start/end audit rows in `error_logs`; per-user cap of 2 mass-import sessions / 24h with graceful UX on the third), **infrastructure** (new Chrome extension distributed via Chrome Web Store under Palateful's developer account — Manifest V3, content script injected into recime.app, popup with single primary CTA, paginated fetch of Recime's internal list-recipes endpoint, per-recipe POST to Palateful with user's API token in `chrome.storage.local`; no new AWS resources, no new env vars on FastAPI; one new Chrome Web Store listing). **Depends on** `epic-recipe-default-books` (Trying Out destination — already shipping per recent /dev commits) and `epic-import-duplicate-detection` (dedup path — also shipping). 30-min lawyer check on TOS framing required pre-launch.
- [epic-nutrition-auto-calc](epic-nutrition-auto-calc.md) — **user sees:** opens any recipe (imported or manually created) and finds new nutrition card below ingredients showing "Per serving: 420 cal · 32g protein · 38g carbs · 18g fat" with "* Estimated based on USDA data" disclaimer. Tapping expands per-ingredient breakdown sheet showing how each ingredient contributes. Unmatched ingredients show "Nutrition unavailable for: heavy cream — [enter manually]" with inline manual-entry path. Recipe edit screen exposes optional override fields (calories / protein / carbs / fat per serving). Settings toggle "Show nutrition" controls global display (default on). — **Touches:** **frontend** (new `NutritionCard` widget on recipe detail, expanded breakdown sheet, manual-macro override fields on recipe edit, missing-ingredient empty state, Settings toggle), **backend** (Alembic migration extending `ingredients` with `nutrition_per_unit` JSON column, one-time data-load script `services/api/scripts/load_usda_nutrition.py` populating ~5000 most-common ingredients from USDA FoodData Central, second migration extending `recipes` with `nutrition_per_serving` + `manual_nutrition_override` JSON columns, new `services/api/src/services/nutrition_calculator.py` summing ingredient nutrition × quantity with unit normalization, recalc-on-save hook on recipe + ingredient edit paths, `GET /v1/recipes/{id}` response shape addition for `nutrition_per_serving`), **infrastructure** (one-time USDA CSV data load via new ops script run on staging then prod after audit-row sign-off; two Alembic migrations; no new AWS resources, no new env vars). **No dependencies**, but recommended after `epic-social-video-import` so freshly-extracted recipes pick up nutrition immediately.

All five epics share the **free-forever positioning bar:** any user-visible copy or surface produced under any epic must honor the free-forever commitment. No "premium," "pro," "upgrade," or "unlock" language. If a story discovers it needs to gate something, the gate must be removed (or feature cut) — not a paywall added.
