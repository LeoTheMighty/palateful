...
`mobile-builds.yml` (master, `3.32.0-0.3.pre`). Tests pass on stable but
...
- The original master pin was `3.32.0-0.3.pre`. **Flutter 3.32.0 GA is
  now on the stable channel**. Pinning to `3.32.0` stable covers the
  same engine surface that the pre-release was reaching for.

Outcome: pin `FLUTTER_CHANNEL: stable` + `FLUTTER_VERSION: '3.32.0'`.
