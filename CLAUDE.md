...
- **`tools/stale-pointer-check.py`** - CI guard (stptr1) that fails when a Markdown file points at a workstream artifact that has moved to the folder-per-artifact layout. A rename sweep cannot cover a file written after it runs, and a new file has no merge conflict to raise. Exempts `## Status log` sections (append-only history) and `evals/` outputs; allowlist in `tools/stale-pointer-allowlist.txt` (`file:lineno:rationale`). Asserts its own coverage and exits 2 rather than passing silently. Proof suite: `tools/stale-pointer-check.suite.py`.
